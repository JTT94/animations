...
class OTScene(MovingCameraScene):
    ...
    def construct(self):
        ...
        ).scale(charts_scale).move_to(LEFT*3).shift(charts_down*DOWN)
        self.add(left_bar)
        ...
        right_bar = BarChart(
            ...
            bar_label_scale_val=0.3,
        ).scale(charts_scale).move_to(RIGHT*3).shift(charts_down*DOWN)

        self.add(right_bar)
        left_tex = Tex(r'Given ', r'$\alpha=\sum_{i=1}^n a_i \delta_{x_i}$',', ', r'$\beta=\sum_{i=1}^m b_i \delta_{y_i}$').scale(tex_scale).move_to(LEFT*3).shift(3.25*UP)
        ...
        tex.generate_target()
        tex.target.shift(2*UP)

        right_bar.generate_target()
        right_bar.target.shift(2*UP).shift(3*LEFT)
        #self.play()
        ...
        self.play(left_bar.animate.rotate(90*DEGREES))
        left_bar.generate_target()
        left_bar.target.shift(1*DOWN)
        ...
        h_sum = Tex(r"$\sum_jp^*_{i,j}=a_i$",color=red).next_to(box_h, RIGHT).shift(0.3*RIGHT)
